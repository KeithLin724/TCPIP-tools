...
namespace KYFuncLite {
	...
	inline SOCKET Fast_TCP_Client_Builder(const std::uint16_t& R_Port, const std::string& IP) {
		...
		if (connect(tcpSock, (sockaddr*)&tcpClient, sizeof(tcpClient)) < 0) {
	...
	inline Socket& UDP_Server(const std::uint16_t& Port) {
		Socket sock;
		try {
			sock.setSocket(Socket::Af_Inet, Socket::Dgram, Socket::Normal);
	...
	inline std::pair<Socket, sockaddr>& UDP_Client(const std::uint16_t& R_Port, const string& IP) {
